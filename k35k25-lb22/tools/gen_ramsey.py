...
Encoding:
  ...
  - optional --cube D (anchor-cell runs): WLOG fix vertex 1's color-1
...
def build(n, colors, swap_break=None, cube=None, vertex_lex=False):
    ...
    if vertex_lex:
        ...
        # ordered 1 < 2 < 3.  Sound: the lex-min of an orbit under the combined group
        # (block vertex perms x color swap) satisfies every such single-element
        # constraint simultaneously, and cube constraints are preserved by the blocks.
        if cube is None:
...
def main():
    ...
    ap.add_argument("colors", help="comma list, e.g. K3,J4,J4")
